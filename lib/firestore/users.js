...
export async function ensureUser(userId) {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      dailyGoal: 20,
      cardsStudiedToday: 0,
      cardsStudiedDate: new Date().toISOString().slice(0, 10),
...
export async function recordStudyActivity(userId, cardsStudied = 0) {
  const user = await ensureUser(userId);
  const today = new Date().toISOString().slice(0, 10);
  ...
  let cardsStudiedToday = user.cardsStudiedToday || 0;
  if (user.cardsStudiedDate !== today) cardsStudiedToday = 0;
  cardsStudiedToday += Math.max(0, cardsStudied || 0);
...
export async function setDailyGoal(userId, dailyGoal) {
  const goal = Math.min(Math.max(Number(dailyGoal) || 20, 5), 200);
  await updateDoc(doc(db, 'users', userId), { dailyGoal: goal });
  return goal;
}

export async function setEmailDigest(userId, enabled, email) {
  const payload = { emailDigest: Boolean(enabled) };
...
export async function getDailyProgress(userId) {
  ...
  return {
    ...
    cardsStudiedToday: studied,
    remaining: Math.max(0, (user.dailyGoal || 20) - studied),
  };
}
