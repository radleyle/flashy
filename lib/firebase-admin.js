...
function getCredential() {
  ...
  if (json) {
    try {
      ...
      console.error('FIREBASE_SERVICE_ACCOUNT_JSON parse failed, trying split fields:', e.message);
    }
  }
  ...
  if (filePath) {
    ...
    if (!existsSync(absolute)) {
      throw new Error(
        `FIREBASE_SERVICE_ACCOUNT_PATH is set to "${filePath}" but that file is not on the server. On Vercel: remove FIREBASE_SERVICE_ACCOUNT_PATH and set FIREBASE_SERVICE_ACCOUNT_JSON (or FIREBASE_CLIENT_EMAIL + FIREBASE_PRIVATE_KEY).`
      );
    }
